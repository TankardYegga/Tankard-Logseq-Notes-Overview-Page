	...
	- Frangipani这篇论文是讲什么的？
	  collapsed:: true
		- Frangipani是一个network file system
			- 类似的有Athena AFS：是一种计算机网络文件系统，用于在分布式计算环境中共享和存储文件。它是麻省理工学院（MIT）开发的一个开源项目，旨在提供可靠的、高性能的文件共享服务。Athena AFS 可以用于大规模的计算集群、校园网络以及其他需要共享文件的环境中。Athena AFS 是一种计算机网络文件系统，用于在分布式计算环境中共享和存储文件。它是麻省理工学院（MIT）开发的一个开源项目，旨在提供可靠的、高性能的文件共享服务。Athena AFS 可以用于大规模的计算集群、校园网络以及其他需要共享文件的环境中。
		- 主要就是在不同的users之间来share file
		- 这个系统有三个核心的ideas，是理解后续的heavy duty transaction system的基础：
			- cache coherence: 缓存一致性
			- distributed locking：分布式锁
			- distributed crash recovery：分布式的故障恢复
	- 一般的网络文件系统Network FS是怎么设计的呢？
	  collapsed:: true
		- ![image.png](../assets/image_1695821247459_0.png)
	  ...
	  collapsed:: true
		...
		- 设计有非常大的区别，[[$red]]==采用的是decentralized的思路==：
			...
			- 这个系统的复杂度实际上来自于客户端FS的complexity了：
				...
				-
		- 让file server code运行在每个客户端所在的机器上，是怎么提高了scalability的？
			- 假设普通的网络文件系统中只有一个file server
				- ![image.png](../assets/image_1695873081494_0.png)
				- 所有的读写操作此时都会被发送给这个单一的file server，这个FS会对相应目录进行查找、执行security checks、打开或者关闭文件等，各种发生在FS系统上的计算都在File server上进行
	  ...
	  collapsed:: true
		...
		- 由这个用例，所驱动的设计选择是什么呢？
			...
			- strong consistency
				- 有时候也称作coherence
			- performance
				- 可以与专门为MapReduce应用所设计的GFS系统进行类比
					- GFS系统的文件不会存放在缓存当中，因为GFS涉及的文件太大了，GFS系统根本无法fit in，且一般来说GFS系统的文件是顺序的；GFS中没有任何关于data本身的缓存，只有一部分用于track chunk servers的位置的缓存，因而GFS中也不存在所谓的缓存一致性问题；
					- 实际上，在本节的论述范围内，GFS甚至算不上是一个真正的文件系统，因为它不能提供这里所需要的vi gcc ,  也就是说它没有提供straight passage或者unique compatibility，而franqipani则可以运行标准的unix系统应用程序，这些应用程序会像在一个单机文件系统下而不是分布式文件系统下一样工作
				...
				- 这个是如何解决Bad Plan中对应的问题的呢？
				  collapsed:: true
					...
					- 问题是，正如前面所说，read事务是从最近的machine里面来读取的，这个machine和其他地理位置的machine会正好组成一个PaxosGroup，很明显当前的machine并不一定是这个PaxosGroup的leader，所以当前的read难道没有可能会读取到stale的data吗？
						...
						- [[$red]]==老师说的是“safe time rule”的：==在时间戳为15的点进行读取操作时，必须要等待一个时间戳比15更大的写入操作，因为这样就可以判断在读取15之前没有进行任何写入操作了，所以就可以安全地来执行此次读取操作，也就不会出现当前的replica看不见在时间戳为10时的写入操作了
							- ![image.png](../assets/image_1695644532866_0.png)
						- [[$red]]==还有另外一个rule的要求：==必要要等待transactions that have prepared but not committed，比如说在时间戳为14的点有一个读写事务已经prepared了但是还没有commit，那么当前时间戳为15时的读取操作必须要等待前面那个事务被commit
							- [[存疑]] 这个rule感觉像是上面那个rule的隐性规则吧？
