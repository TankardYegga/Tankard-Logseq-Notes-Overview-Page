...
- GFS：
  collapsed:: true
	...
	- GFS client 和 application 之间的关系？
		- application可能是multiple map jobs 或者 multiple reduce jobs，application与GFS library进行连接；所以，GFS并不是用于编辑或者compile文件的系统，而是可以看作是用于大量计算的special reverse file system
		- GFS真正想要实现的目标是 单个应用能够处理impressive的数据量（单个disk （MB, megabytes）× 机器数目）
	- GFS要怎么理解它的设计？
	  collapsed:: true
		- ![image.png](../assets/image_1688350994614_0.png)
			- GFS client 怎么与master进行通信的？（整个文件系统工作的核心plan）
				- GFS client会询问master关于要处理的某个文件的某个chunk的相关信息：(filename, chunk index) 比如文件A的第二个chunk（如果chunk是从0开始编号的，那就是chunk 1）；master会返回给client这个chunk 1的handle以及存储该chunk的所有的chunk server的位置; 多个应用可能读取同一个应用的不同chunk block，比如应用A读取chunk 1， 应用B读取chunk 2
				- 一旦client得到了（chunk handle, a list of chunk locations），就会以当时的网络传输速度直接从chunk server的对应disk上读取数据到应用中；多个GFS client可以从多个disk中同时读取数据，我们就会得到tremendous performance（出色的性能）
					- 一个64MB的chunk block在linux的file system当中是以linux file的形式存在的
		- ![image.png](../assets/image_1688139960528_0.png)
			- 图中的primary和secondaries是对list of chunk servers的划分吗？
				- 是的，通常 每个chunk的份数是3，一份是primary，两份是secondaries
			- log的作用是什么呢？master先执行对客户端的响应再写入log，还是先写入log再响应客户端，为什么呢？为什么要使用checkpoint技术呢？
			  collapsed:: true
				- 如果新增了一个文件，那么GFS中需要新增加一个namespace；如果文件的存储位置移动了，那么GFS中的filename到chunk blocks的mapping也要进行改变；无论是哪种造成GFS改变的操作，都需要写入到log中，log是一种stable storage
				- master在向client回应这些操作已经被执行前，会先将其写入log中；因为当master执行失败（fail）或者出现故障了(crash)再重启的话，可以读取log然后再重建内部状态，但是如果先响应client而后master fail or crash了的话，比如响应client说文件已经创建但是master fail了，那么实际上这个文件并没有在GFS系统中被创建，client因为就会得到一些奇怪的结果；[[#red]]==[[$red]]这是某种程度上的consistency==
				- 因为replay总是把日志中从最开始到现在的所有操作重新执行一遍，这意味着如果master crash了，那么系统服务将会长时间不可用(be down for a long time),  所以需要在stable storage中存储checkpoint，master会定期地将自己的external state和从file namespace到array of chunk handles的mapping进行checkpoint，那么replay时只需要执行log中从last checkpoint到crash时的所有操作即可，这使得recovery操作变得非常快速
			- GFS的master设计的基本逻辑是什么？
			  collapsed:: true
				- ![image.png](../assets/image_1688179386878_0.png)
					- master作为控制中心，存储chunk数据的元信息
			- 为了master能够正确地工作，what state should be end up in stable storage ? 也就是稳定存储介质中应该存储哪些状态呢？
			  collapsed:: true
				- 从文件名到chuck handle的mapping需要被稳定存储吗？
					- 学生回答的是：如果master crash了，master可以从log中获取信息来重建这些mapping并加载到main memory，所以只需要log被存储在hard storage中
					- 老师回答：this array of chunk handles需要被放入stable storage，否则的话，当我们创建一个新文件而没有把它写入stable storage，这个文件就会丢失
					- 老师和学生的答案其实是一致的，因为log就是对这个mapping的stable storage
				- GFS中从文件名到chuck handle的mapping是以什么数据结构来存储的？
					- 在 GFS 中，从文件名到 chunk handle 的映射是使用一种称为"file table"的数据结构来存储的
					- File table 是一个维护文件元数据的表格，其中每一行包含文件名、文件属性以及一组 chunk handle，用于表示文件的分块。这种数据结构可以使用哈希表或者类似的索引结构来实现，以提高文件查找的效率。每当文件新增、删除或修改时，GFS 的 Master 负责更新并维护这个 file table，使之保持最新的映射关系和文件元数据。另外，GFS 还使用 GFS Chunkservers 来存储具体的 chunk 数据，并通过 chunk handle 来定位和访问文件的不同数据副本
					- 在 GFS 的 file table 中，文件名 (filename) 通常被用作主键，以便快速进行文件查找和索引。每个文件都有唯一的文件名，在 file table 中，文件名将用作索引，以便在需要获取文件的元数据或处理文件操作时能够快速定位到相应的条目。通过使用文件名作为主键，可以有效地管理和维护文件的映射关系。当需要操作特定文件时，可以通过查找文件名来快速定位到相应的文件记录，并获取其相关的信息和 chunk handle。
				- GFS中的同一个chunk的三个备份对应的是同一个chunk handle吗
					- 在 Google 文件系统（GFS）中，对于同一个 chunk 的三个备份，它们共享同一个 chunk handle。Chunk handle 是一个唯一标识符，用于唯一地标识一个 chunk。GFS 使用这个标识符来定位和操作存储在文件系统中的 chunk。无论是原始数据 chunk 还是备份 chunk，它们都会有相同的 chunk handle。这样可以方便地跟踪和管理 chunk 的多个备份以确保数据的可靠性和容错性。
				- GFS中master中同一个文件的三个副本是有三个从文件名到chunk handle的mapping, 还是共享同一个从文件名到chunk handle的mapping呢？
					- 在 Google 文件系统（GFS）中，Master 维护的文件元数据中，同一个文件的三个副本实际上共享同一个从文件名到 chunk handle 的映射。具体来说，文件的元数据中会存储文件名以及由多个 chunk handle 组成的 chunk 列表。每个 chunk handle 表示文件的一个分块，而这个分块对应一个具体的数据副本。因此，文件名到 chunk handle 的映射是针对整个文件而言，并且所有的副本共享同样的映射关系。这样设计的目的是为了简化文件的访问和管理，同时保证数据的可靠性和冗余备份
				- GFS 中 从chunk handle到lease time的mapping 是用作什么的？
					- 在 GFS 中，从 chunk handle 到 lease time 的映射被用于管理文件的读写操作。每个 chunk 都有一个对应的 chunk handle，它是一个唯一的标识符，用于标识和访问特定的数据块。
					- 通过映射表（也称为 lease table），GFS 跟踪每个 chunk 的租约（lease）时间。租约是用于管理对 chunk 的独占访问权的机制。当一个客户端想要读取或写入一个 chunk 时，它首先会请求获取该 chunk 的租约。租约的持有者将获得对该 chunk 的独占访问权，通过这种方式，GFS 可以确保同一时间只有一个客户端可以对一个 chunk 进行写入操作。
					- lease time（租约时间）表示当前租约的有效期限。如果一个客户端获取了一个 chunk 的租约，并且在有效期内保持持有该租约，那么它可以继续对该 chunk 进行读取或写入操作。一旦租约过期，其他客户端可以请求获取该 chunk 的租约，并且只有获取了租约的客户端才能对该 chunk 进行操作。
					- 通过维护 chunk handle 到 lease time 的映射，GFS 可以追踪并管理每个 chunk 的租约情况，确保对文件的并发访问是可控的，并且保证了一致性和可靠性
				- lease table大概是怎么设计的?
					- 租约表（lease table）的设计可以根据具体的系统要求和需求的复杂程度而有所差异。下面是一种简单的设计示例：
						- 1. 数据结构：租约表可以使用哈希表（Hash table）的数据结构来存储 chunk handle 到 lease time 的映射关系。哈希表具有高效的查找和插入操作，可以快速地定位到指定 chunk handle 的条目。
						- 2. 存储方式：租约表可以存储在内存中，以提高数据访问的速度。对于大规模的文件系统，租约表也可以存储在磁盘上，并使用缓存技术进行读写加速。
						- 3. 条目格式：每个租约表的条目包含以下信息：
							- Chunk handle：用于唯一标识一个 chunk 的标识符。
							- Lease time：租约的有效期限，可以是一个时间戳或一个持续时间。
							- Lease holder：持有该租约的客户端或服务器。
						- 4. 租约管理：当一个客户端请求获取一个 chunk 的租约时，租约表会检查该 chunk 是否已经被其他客户端持有租约。如果没有，则将新的 lease time 和 lease holder 更新到租约表中。如果已经被其他客户端持有，策略可以包括等待、获取失败或者后续的竞争策略。在操作完成后，租约表会更新租约的 lease time 或从租约表中移除该条目。
						- 5. 定期清理：为了避免租约表过大或租约过期未释放占用资源，可以定期进行租约表的清理操作。清理可以基于 lease time 过期时间进行，过期的租约条目可以从租约表中移除或标记为无效。
					- 注意，这只是租约表的简单设计示例，实际的设计会根据具体的系统需求和性能要求进行调整和扩展。
				- 那么从chunk handle 到 chunk server 的列表的 mapping 是否需要长期存储？
					- 不需要，因为它只是一个volatile state
						- 在计算机科学中，"volatile state" 通常指的是内存中存储的数据或对象的状态，可能由于外部因素或并发操作的存在而发生变化
					- 在论文中说当master进行reboot时，它会要求chunk servers告诉master他们所拥有的chunks是什么
					- 同样，从chunk handle到primaries, secondaries 的映射，从chunk handle到lease time的mapping，都不需要长期存储
				- master需要将chunk handle对应的verison number进行stable storage吗？也就是master需要对chunk handle的version number进行记忆吗
					- 是的，需要记忆。因为这可以告知我们，当整个系统崩溃了，然后重新恢复时，哪个chunk server具有最新（up-to-date）的信息；具体来说，当master崩溃后恢复时（come back up），它会尝试连接所有的chunk server，可能会有部分chunk server仍然处于宕机的状态当中，然后它会尝试找出每个chunk handle在chunk server中的maximum version number
			-
	- GFS的读取流程是怎么样的？
	  collapsed:: true
		- ![image.png](../assets/image_1688202216156_0.png)
		- ![image.png](../assets/image_1688390193776_0.png)
			- 为什么需要在client端进行chunk server list的缓存呢？
				- 因为master是单点的一台机器，只有有线的内存和有限的网络接口，当client能够使用cache时，能够减少master与client之间的通信负载，同时使得client端读取数据的速度变得更快
			- 为什么client从最近的chunk server来进行读取呢？
				- 为了minimize network traffic，所以可以pump as much data through the clients as possible and maximum the throughput；我们需要跨越数据中心网络，该网络可能被一些关键处的频繁链接所淹没（swamped）而造成延迟
			- 是否有注意的细节呢?
				- ![image.png](../assets/image_1688392493453_0.png)
					- chunk server检查chunk 版本的原因是为了确保没有stale data（因过时而无效的数据），我们需要尽可能地降低过时的发生频率
	- GFS的写入流程是怎么样的？
	  collapsed:: true
		- 流程图：
			- ![image.png](../assets/image_1688396211207_0.png)
		- GFS为什么只支持追加（Append）而不支持修改（Update）?
			- 因为追加操作更容易保持一致性
		- Step 1: Client向master发送写请求后，master会做什么？
			- 会找到对应file的chunk handle和chunk server list
			- 然后：
				- 随机选择chunk server list中的一个chunk作为primary，其他作为secondaries
				- master会grant这个primary一个lease，这个lease会有一个过期时间
				- master给这个primary和secondaries都增加一个version number（每次新选出一个primary，就意味着文件系统进入一个new epoch），改动后的primary和secondaries可以看成是一个new replica group
					- 请问此时primary和secondaries需要存储version number到它们自身的disk上面去吗？
						- 是的，因为如果chunk server把version number写在内存的话，当chunk server崩溃后再重启，它就不知道它原先所在的replica group了，此时也无法说服master当前的primary具有最新的数据信息了
						- 总之，version number在master和chunk server对应的disk上都要进行写入。当master接收到primary和secondaries都已经写入version number、primary已经接收到了lease的acknowledgement（确认）时，就会在自己的disk中写入version number然后respond to client
		- Step 2: master 返回什么？
			- primary + secondaries + version#
		- Step 3: client 发送的数据是怎么进行传递的？
			- client 会首先给它所知道的closest secondary发送数据，然后数据会不断发送给其他的数据副本；一旦第一个secondary接收到一些数据，它立马就把数据传递给下一个邻近的replica
			- 这样进行设计的原因是：client所拥有的单个网络接口就像一个full network interface一样被使用
		- Step 4：
			- client试图向primary追加数据，具体会怎么做呢?
				- 首先会check primary的version和client中缓存的version是否一致
				- 如一致，然后client会check 这个primary的lease是否valid
				- 如valid，就会计算primary中对应的写的offset
				- primary执行写入数据的操作
		- Step 5 and 6 ：
		  collapsed:: true
			- primary向其他的secondaries发送待写入的record和写入的位置
			- 若所有的secondaries都写入成功，就会respond to primary类似：“append successfully”的message；但是也有可能fail，比如primary与某一个secondary的连接失败了（可能是secondary crash了也可能是该secondary的网络出现了问题），只要有一个secondary的追加写入失败了，primary就会给client回应error，此时client将会reissue this event and retry
				- 这种写入方式有什么特点呢？
					- at least once
				- 如果retry，primary还会从同一个offset开始写数据吗？
					- 会设置一个新的offset开始写，这样就会出现下图所示的问题：同一个数据在同一个chunk server中被写入了两次，这通常来说是不方便的，表现出来的结果就是：邮件中的body被写了两次、a1a1(两次a1）这种
						- ![image.png](../assets/image_1688400081666_0.png)
					- 那么，要怎么解决呢？
						- application其实不是和GFS直接交互的，而是通过library，当写入数据时，library可以使得写入的位置有一个unique id，那么后续重新开始写时，如果id出现了重复，就会跳过这个chunk block不写了；
						- 可以加上checksum来进一步保证不会重复写
						- application不需要对此进行处理，是由library和GFS共同来实现的
				- master、chunk server和client都是trusted的吗？
					- 是的，包括google写入的程序都是trusted，它就是作为一个内部文件系统被使用的。并不像linux文件系统那样有权限控制
						- "trusted file system"（可信文件系统）是指在安全方面经过验证和认可的文件系统。它提供了对文件和数据的保护和安全性，以防止未经授权的访问、数据泄露或文件损坏
		- Other Questions:
		  collapsed:: true
			- master一定会或者说不得不读取到stale data吗？因为client从master获取到的version number非常容易过期（有其他client会修改这些chunk，从而修改了对应chunk的version number和chunk handle对应的version number），也就是说client缓存到的version number的最新性的这个时长是无法确定的，“最新性”的市场完全取决于当时多个client对相关的file chunk的读取和写入情况 和 lease设定的primary的过期时间
			  collapsed:: true
				- 这个问题比较复杂，可以通过下面两张图所示的具体场景来进行解释：
					- ![image.png](../assets/image_1688447278619_0.png){:height 383, :width 656}
					- ![image.png](../assets/image_1688447919635_0.png)
				- 对于上面这个场景的解释是：
				  collapsed:: true
					- Client C1 目前已经cache了来自master的version number = 10，P1、S1、S2分别是client C1待写入的file的primary chunk和secondary chunks，但是目前S2 crash or network broke down了，这就造成了P1和S1无法与S2进行通信了
					- 假设经过一轮lease，当前的primary超时了，那么master就会重新pick a new primary, 那么master会给对应的chunk handle的version number自增1（10 +1 = 11），然后master就会给P1和S1发送message告诉它们两对应的chunk block的version number也应该是11，而由于S2宕机还未恢复，master与S2无法进行通信，所以可能会选择其他的chunk block来作为一个new secondary
					- 此时另外一个Client C2 需要请求对同样的文件写入数据，由于其发送请求的顺序靠后，它刚好得到了来自master的最新的version number（11），此时C2和P1的version number匹配，于是它向P1写入了最新数据，P1将数据和offset告知S1从而S1也实现了同样的完全一致的数据写入操作；而S2由于无法与P1通信，所以没有进行任何新的写入操作，此时可能替代S2的其他secondary更新了数据
					- 此时C1想要向P1、S1、S2写入数据，但是P1和S1的version number是11，与C1的10并不匹配，所以P1和S1并没有写入；此时若S2重启成功了，那么C1可以直接与S2进行通信，且因两者version number都是10，那么S2会写入成功，并会返回给client一个确认。
					- 很明显，此时的C1读取到了一个stale data
				- 关于上面场景解释的几个问题：
				  collapsed:: true
					- 为什么P1的11和S1的11无法直接返回给Client C1呢？这样C1不就可以有最新的version number了吗？
					  collapsed:: true
						- 因为client中的version number会缓存一个更长的时间，而且论文的协议并没有提供相关的机制来实现这一点
					- 当系统尝试向S2推送并更新数据时，并且不能做到的时候，会自增version number吗？[[存疑]]
					  collapsed:: true
						- master所存储的version number，只有当pick a new primary时才会发生变化，而client向primary和secondaries写入数据时的成功或者失败不会影响master的version  number
						- 而关于primary和secondaries的version number：最先开始是由primary的租约变更，导致master会将自增后的version number作为消息msg传递给primary和secondaries，而只有primary和secondaries的数据都写入成功后，才会把这个msg更新到primary和secondaries的version number里面，如果写入失败，则不更新
			- primary要怎么确认它已经和所有的secondaries通信过了呢？因为只有这样，primary才能知道什么时候write完全成功了
				- master会告知该primary所有需要去update的secondaries
			- 当master basically issue the lease to the primary时，如果one of the secondaries在那时候刚好宕机了，那么master会将这个看作是一个failure或者master只是对所有还alive的secondaries的version number自增1，并且忽略宕机的这些secondaries因为它们不管如何都会有一个过期的版本号呢？
			  collapsed:: true
				- 论文中关于recovery和reconfiguration的部分是如何工作的，其实是比较模糊的（vague）
				- 可能：primary会向所有的secondaries和自己发送heartbeats，超过一定时间它能够判断出某一个secondary比如s2宕机了；当前primary的lease可能会超时而过期，于是会创建出一个新的primary
				-
			- GFS在追加已有的文件时，应该是不存在所谓的数据流吧？应该只有控制流，也就是primary把待追加的记录和offset传递给其他secondaries，其他secondaries执行数据写入操作即可，对吗？
			  collapsed:: true
				- 您是正确的，对于GFS中的追加写操作，确实不存在实际的数据流。相反，它主要涉及控制流。
				- 当客户端执行追加写操作时，客户端会将待追加的记录发送给主节点（primary）。主节点负责协调和管理文件的写入操作。
				- 主节点会接收到待追加的记录，并确定记录需要添加到文件的哪个位置（即偏移量offset）。然后，主节点将记录的内容和偏移量信息传递给其他辅助节点（secondaries）。
				- 其他辅助节点根据主节点的指示，将记录写入到文件的相应位置。这个过程通过控制流进行调度和协调，不需传递实际的数据流。
				- 这种设计可以减少数据的传输和复制次数，提高写入操作的效率和性能。而且，由于只涉及控制流的传递，而不是实际数据的传输，所以可以大大减少网络传输的开销。
			- GFS是不是只有创建新文件时，才会出现数据流从client到secondaries到primary再到其他secondaries的流动？追加写已有的文件时，数据流是不经过client的吧？
			  collapsed:: true
				- 在GFS中，创建新文件时确实会有数据流从客户端到辅助节点（secondaries），再到主节点（primary），最后到其他辅助节点的流动。这是因为在创建新文件时，需要选择一个主节点来负责该文件的管理和数据分配。辅助节点负责保存文件的副本，并与主节点保持同步。
				- 当追加写已有的文件时，数据流经过的路径可以有所不同。在这种情况下，主节点将会知道文件的位置并负责协调数据的写入。通常情况下，数据不需通过客户端传递，而是通过直接与主节点进行通信，将数据写入到文件所在的位置。
				- 这种直接与主节点交互的方式可以提高效率，减少了数据的传输路径和延迟，并且允许主节点更好地管理并控制文件的写入操作。
				- 总之，GFS的数据流路径在创建新文件和追加写已有文件时可能会有所不同，主要是因为不同的操作需要使用不同的协调和路径来处理数据的存储和同步。
	- GFS 中应该怎么思考一致性问题？
	  collapsed:: true
		- 思考一致性问题，就是要思考所有可能的failures，这些failures将会导致inconsistency
		- 可能的Failure有：
			- master给chunk servers发送heartbeats，但是没有get response
			  collapsed:: true
				- ![image.png](../assets/image_1688463092629_0.png)
			- 哪些情况下master需要pick a new primary?
				- 当lease过期时，
					- 因为如果同时出现两个primary chunk server, 这个设计将会是很糟糕的，因为client写入数据时不知道该向哪一个primary开始写入record，而master也不会知道哪一个chunk才算是真的primary，从而会导致租约过程出现混乱；
					- 两个master或者两个primary的situation又被称作split brain syndrome
					-
		-
	- GFS 中怎么获得更强的consistency？[[没太理解]]
	  collapsed:: true
		- ![image.png](../assets/image_1688464019866_0.png)
